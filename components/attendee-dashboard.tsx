...
export function AttendeeDashboard() {
  ...
  return (
    <DashboardLayout title="Discover Events" badges={["Find Your Next Experience"]}>
      ...
      <div className="grid grid-cols-[1fr_300px] gap-8">
        <div>
          <h3 className="font-bold text-slate-800 uppercase text-xs tracking-widest mb-4">Upcoming Events</h3>
          {loading ? (
            ...
            <div className="grid grid-cols-2 gap-6">
              {events.map(event => {
                 ...
                 return (
                   <div key={event.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md hover:border-purple-300 transition-all group flex flex-col">
                    <div className="h-40 bg-slate-200 relative overflow-hidden shrink-0">
                       {event.imageUrl ? (
                         ...
                         <div className={`absolute inset-0 opacity-90 group-hover:scale-105 transition-transform duration-500 ${event.title.length % 2 === 0 ? 'bg-gradient-to-br from-purple-500 to-indigo-600' : 'bg-gradient-to-br from-blue-500 to-cyan-600'}`}></div>
                       )}
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      <h4 className="font-bold text-lg text-slate-900 leading-tight mb-2 line-clamp-2">{event.title}</h4>
                      <div className="space-y-1.5 text-xs text-slate-500 font-medium mb-4">
                        <div className="flex items-center gap-2">
                          <Calendar className="w-3.5 h-3.5 text-purple-500 shrink-0" />
                          <span>{eventDate.toLocaleString(undefined, { weekday: 'short', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit'})}</span>
                        </div>
                        <div className="flex items-center gap-2">
                      ...
                      <Link href={`/events/${event.id}`} className="mt-auto w-full bg-slate-50 hover:bg-purple-50 text-slate-700 hover:text-purple-700 font-semibold py-2 rounded-lg flex items-center justify-center gap-2 transition-colors border border-slate-100">
                        View Details
        ...
        <div>
          <h3 className="font-bold text-slate-800 uppercase text-xs tracking-widest mb-4">Calendar Map</h3>
