               ...
               return (
                 <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                   <div className="flex items-center gap-2 mb-1">
                     <span className="text-xs font-bold text-slate-700">{isMe ? "You" : msg.senderName}</span>
                     {!isMe && (
                       <span className={`text-[9px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded ${
                         msg.senderRole === 'admin' ? 'bg-purple-100 text-purple-700' :
                         msg.senderRole === 'organizer' ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-600'
                       }`}>
                         {msg.senderRole}
                       </span>
                     )}
                   ...
                   <div className={`px-4 py-2.5 rounded-2xl max-w-[80%] text-sm shadow-sm ${
                     isMe ? 'bg-purple-600 text-white rounded-tr-sm' : 'bg-white border border-slate-200 text-slate-800 rounded-tl-sm'
                   }`}>
                     {msg.text}
                   </div>
