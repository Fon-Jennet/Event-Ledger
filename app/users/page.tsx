  ...
  if (!profile || profile.role !== "admin") {
    return (
      <DashboardLayout title="User Management">
         <div className="p-8 text-center text-slate-500">Unauthorised access. Admins only.</div>
      </DashboardLayout>
    );
  }
                  ...
                  <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-bold text-slate-900 mb-0.5 flex items-center gap-2">
                            {u.name}
                            {isSuperAdmin && <ShieldAlert className="w-3.5 h-3.5 text-purple-600" title="Super Admin" />}
                          </div>
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${
                        ...
                        u.role === 'organizer' ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-700'
                      }`}>
                        {u.role}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      ...
                      {isSuperAdmin && <span className="text-xs text-slate-400 font-medium">Protected Role</span>}
                    </td>
                  </tr>
                );
              })}
